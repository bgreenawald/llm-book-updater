"""
Build script for Beyond Good and Evil (Timeless Library Edition).
...
class BeyondGoodAndEvilBuilder(BaseBookBuilder):
    """
    Book-specific builder for Beyond Good and Evil.
    ...
    def get_source_files(self) -> Dict[str, Path]:
        """
        Get the source file paths for Beyond Good and Evil.
        ...
        """
        source_files = {
            "modernized": self.config.source_output_dir / "03-input_transformed Final_two_stage_1_fixed.md",
            "annotated": self.config.source_output_dir / "06-input_transformed Annotated_1_fixed.md",
        }
    ...
    def get_original_file(self) -> Optional[Path]:
        """
        Get the original source file for Beyond Good and Evil.
...
def build(version: str, name: str):
    """
    Build function for Beyond Good and Evil.
    ...
    config = BookConfig(
        name=name,
        version=version,
        title="Beyond Good and Evil (Timeless Library Edition)",
        author="Friedrich Nietzsche",
    )

    # Create and run the builder
    builder = BeyondGoodAndEvilBuilder(config=config)
    builder.build()
