"""Build script for Democracy in America (Timeless Library Edition).
...
class DemocracyInAmericaBuilder(BaseBookBuilder):
    """Book-specific builder for Democracy in America.
    ...
    def get_source_files(self) -> dict[str, Path]:
        """Get the source file paths for Democracy in America.
    ...
    def get_original_file(self) -> Optional[Path]:
        """Get the original source file for Democracy in America.
...
def build(version: str, name: str) -> None:
    ...
    builder = DemocracyInAmericaBuilder(config=config)
    builder.build()
