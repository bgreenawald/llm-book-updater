"""
Build script for The Road to Serfdom (Timeless Library Edition).
...
class TheRoadToSerfdomBuilder(BaseBookBuilder):
    """
    Book-specific builder for The Road to Serfdom.
    ...
    def get_source_files(self) -> Dict[str, Path]:
        """
        Get the source file paths for The Road to Serfdom.
    ...
    def get_original_file(self) -> Optional[Path]:
        """
        Get the original source file for The Road to Serfdom.
...
def build(version: str, name: str):
    """
    Build function for The Road to Serfdom.
    ...
    config = BookConfig(
        name=name,
        version=version,
        title="The Road to Serfdom (Timeless Library Edition)",
        author="Friedrich Hayek",
    )

    # Create and run the builder
    builder = TheRoadToSerfdomBuilder(config=config)
    builder.build()
