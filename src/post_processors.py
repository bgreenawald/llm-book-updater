...
class PreserveFStringTagsProcessor(PostProcessor):
    ...
    def process(self, original_block: str, llm_block: str, **kwargs) -> str:
        """
        Restores lines that are exactly a tag if they were removed by the LLM, and also restores any immediately adjacent blank lines that were deleted, to preserve the original block structure and spacing.
        Args:
        ...
        if has_similarity and not has_multiple_occurrences:
            # If there's similarity and no multiple occurrences, try to restore tags and adjacent blank lines at their original positions
            processed_lines = list(llm_lines)
            for tag, i1, i2, j1, j2 in reversed(matcher.get_opcodes()):
                if tag == "delete":
